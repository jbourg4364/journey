          ...
          <div className="ind-parent">
            {carlineParents.map((child) => (
              <li key={child.id}>
                ...
                <hr></hr>
                <p className="pickedup">Picked up?</p>
                <input
                  className="pickedup"
                  type="checkbox"
                  onChange={() => handleStatusChange(child.id, child.children)}
                  id="checkbox"
                  key={child.id}
                />
              </li>
            ...
            <div>
              {filteredPickedUp.map((name) => (
                <li key={name.id} className="pickedup-ind">
                  {name.allChildren} | {`${formatDate(name.pickedupdate)}`} |{" "}
                  {`${formatTime(name.pickedupdate)}`}
                </li>
              ))}
            </div>
          ) : (
